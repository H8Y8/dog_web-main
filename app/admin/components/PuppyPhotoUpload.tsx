...
import { PuppyPhotoType, PUPPY_PHOTO_TYPE_LABELS } from '@/lib/api/puppyPhotos'
...
interface PuppyPhotoUploadProps {
  ...
  /** 是否允許多檔案上傳（cover只能單檔，其他可多檔） */
  multiple?: boolean
  /** 已存在的照片URL陣列（用於顯示現有照片） */
  existingPhotos?: string[]
  /** 檔案選擇完成的回調 */
  onFilesSelected?: (files: File[], photoType: PuppyPhotoType) => void
  /** 檔案壓縮完成的回調 */
  onFilesCompressed?: (files: File[], photoType: PuppyPhotoType) => void
  /** 刪除現有照片的回調 */
  onDeleteExisting?: (photoUrl: string, photoType: PuppyPhotoType) => void
  /** 自定義CSS類名 */
  className?: string
  /** 是否禁用 */
  disabled?: boolean
...
const PUPPY_PHOTO_TYPE_DESCRIPTIONS: Record<PuppyPhotoType, string> = {
  cover: '單張主要照片，建議尺寸 800x600px，展示幼犬最佳狀態',
  album: '多張生活照片，展示幼犬的日常活動和成長過程',
  pedigree: '血統證書文件照片，需要清晰可讀所有文字',
  health_check: '健康檢查報告或疫苗證明照片'
}

export default function PuppyPhotoUpload({
  photoType,
  multiple = photoType !== 'cover',
  existingPhotos = [],
  onFilesSelected,
  onFilesCompressed,
  onDeleteExisting,
  ...
  const handleFiles = useCallback(async (files: FileList) => {
    ...
    for (const file of fileArray) {
      ...
      if (!isValidFileSize(file, 5)) { // 幼犬照片限制為5MB
        alert(`檔案 "${file.name}" 過大（最大 5MB）`)
        continue
      }
    ...
    // 如果是cover類型，只允許一個檔案
    const filesToProcess = photoType === 'cover' ? [validFiles[0]] : validFiles
  ...
  const compressFiles = useCallback(async (files: FileWithPreview[]) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        try {
          // 為幼犬照片使用適當的壓縮設置
          const compressionType = photoType === 'cover' ? 'avatar' : photoType
          const compressed = await compressImageByType(file, compressionType)
    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div>
        <h3 className="text-lg font-medium text-gray-900">
          {PUPPY_PHOTO_TYPE_LABELS[photoType]}
        </h3>
        <p className="text-sm text-gray-600">
          {PUPPY_PHOTO_TYPE_DESCRIPTIONS[photoType]}
        </p>
      </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {existingPhotos.map((photoUrl, index) => (
              <div key={index} className="relative group">
                <img
                  src={photoUrl}
                  alt={`${PUPPY_PHOTO_TYPE_LABELS[photoType]} ${index + 1}`}
