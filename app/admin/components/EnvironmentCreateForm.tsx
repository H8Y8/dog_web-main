...
import { CreateEnvironmentInput, EnvironmentType, ENVIRONMENT_TYPE_LABELS } from '../../../lib/types/environment'
...
export default function EnvironmentCreateForm({ onSuccess, onCancel }: EnvironmentCreateFormProps) {
  const { addEnvironment } = useEnvironments()
  const { session } = useAuth()
  
  const [formData, setFormData] = useState<CreateEnvironmentInput>({
    name: '',
    type: EnvironmentType.ACCOMMODATION,
    description: '',
    features: []
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await addEnvironment(formData, session?.access_token)
      ...
      if (result.success && result.data) {
        ...
        setSubmitError(result.error?.message || '新增環境設施失敗')
      }
    } catch (error) {
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900">新增環境設施</h2>
        {onCancel && (
          <button
            onClick={onCancel}
