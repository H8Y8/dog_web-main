...
import { CreateMemberInput, MemberRole, MemberStatus, Gender, ROLE_LABELS, STATUS_LABELS, GENDER_LABELS } from '../../../lib/types'
...
export default function MemberCreateForm({ onSuccess, onCancel }: MemberCreateFormProps) {
  ...
  const [formData, setFormData] = useState<CreateMemberInput>({
    ...
    gender: Gender.MALE,
    color: '',
    role: MemberRole.TRAINING,
    status: MemberStatus.ACTIVE,
    description: '',
    personality_traits: ''
  })
  
  const [errors, setErrors] = useState<Partial<CreateMemberInput>>({})
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    // 清除對應欄位的錯誤
    if (errors[name as keyof CreateMemberInput]) {
      setErrors(prev => ({
        ...prev,
        [name]: undefined
      }))
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<CreateMemberInput> = {}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await addMember(formData, session?.access_token)
